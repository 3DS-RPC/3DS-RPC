...
class APIClient:
    ...
    async def update_presence(self, user_data: UserData, network: NetworkType):
        ...
        r = await client.post('%s/users/@me/headless-sessions' % API_ENDPOINT, data=json.dumps(data), headers=headers)
        r.raise_for_status()
    ...
    async def reset_presence(self):
        ...
        r = await client.post('%s/users/@me/headless-sessions/delete' % API_ENDPOINT, data=json.dumps(data), headers=headers)
    ...
    async def refresh_bearer(self):
        ...
        json_response = await client.post('%s/oauth2/token' % API_ENDPOINT, data=data, headers=headers)
